...
internal abstract class BaseAsyncRepository<TEntity> where TEntity:class,IEntity
{
    ...
    protected virtual async Task<List<TEntity>> ListAllAsync() => await Entities.ToListAsync();
    
    protected virtual async Task AddAsync(TEntity entity) => await Entities.AddAsync(entity);
